...
**Niveau :** Débutant  
**Prérequis :** Connaissances de base en électronique  

**Objectifs pédagogiques :**
- Installer et configurer KiCad 9.0
- Maîtriser le workflow de conception d'un PCB
- Créer un schéma électronique complet
- Créer des symboles et empreintes personnalisés
- Router un circuit imprimé 2 couches
- Générer les fichiers de fabrication (Gerber, perçage)
...
2. [Téléchargement et installation de KiCad](#2-téléchargement-et-installation-de-kicad)
3. [Concepts de base et workflow](#3-concepts-de-base-et-workflow)
4. [Partie 1 : Création du projet](#4-partie-1--création-du-projet)
5. [Partie 2 : Dessin du schéma](#5-partie-2--dessin-du-schéma)
6. [Partie 3 : Création de symboles personnalisés](#6-partie-3--création-de-symboles-personnalisés)
7. [Partie 4 : Création d'empreintes personnalisées](#7-partie-4--création-dempreintes-personnalisées)
8. [Partie 5 : Conception du circuit imprimé](#8-partie-5--conception-du-circuit-imprimé)
9. [Partie 6 : Génération des fichiers de fabrication](#9-partie-6--génération-des-fichiers-de-fabrication)
...
- **AHT10** : capteur de température et humidité (communication I²C)
- **Écran OLED 0.96" (SSD1306)** : affichage des données (I²C)
- **2× Résistances 4.7kΩ** : résistances de pull-up pour le bus I²C
- **Connecteur d'alimentation** : alimentation externe

**Particularité pédagogique :**  
Les composants **AHT10** et **OLED** n'existent pas dans les bibliothèques standard de KiCad. Vous apprendrez donc à créer vos propres symboles et empreintes à partir des datasheets fournis.

---

## 2. Téléchargement et installation de KiCad

### 2.1 Téléchargement

Rendez-vous sur : **[https://www.kicad.org/download/](https://www.kicad.org/download/)**

Téléchargez **KiCad 9.0** (version stable) pour votre système :
- **Windows** : Installateur `.exe`
- **Linux** : Gestionnaire de paquets de votre distribution
- **macOS** : Fichier `.dmg`
...
2. **Important** : Cochez l'installation des bibliothèques par défaut
...
#### macOS
1. Ouvrez le `.dmg`
2. Glissez KiCad dans Applications

### 2.3 Configuration au premier lancement

Au premier démarrage de KiCad, vous devrez configurer les **tables de bibliothèques**.

**Table de bibliothèques de symboles :**
1. Une boîte de dialogue apparaît
2. Sélectionnez : **"Copier la table de bibliothèques de symboles globale par défaut (recommandé)"**
3. Si désactivée, choisissez l'option personnalisée et naviguez vers :
...
**Table de bibliothèques d'empreintes :**
- Même procédure pour le fichier `fp-lib-table`
